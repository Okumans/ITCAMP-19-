...
function TodoInfo() {
  ...
  const addTodoItem = async () => {
    ...
    try {
      const createdItem = await pb.collection("todoItem").create(toAddTodo)

      const LinkedItem  = {
        title: todo.title,
        description: todo.description,
        "todoItem": [...(await pb.collection("todo").getOne(id)).todoItem,
          createdItem.id
      ]}
      console.log(LinkedItem)

      const newToAddTodo = await pb.collection("todo").update(id, LinkedItem)
